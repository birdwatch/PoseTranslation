...
from src.libs.loss_fn.focal import FocalLoss
from src.libs.loss_fn.tmse import TMSE, GaussianSimilarityTMSE
...
class ActionSegmentationLoss(nn.Module):
    ...
    def __init__(
        ...
        if ce:
            self.criterions.append(nn.CrossEntropyLoss(weight=weight))
        ...
        if gstmse:
            self.criterions.append(GaussianSimilarityTMSE(threshold=threshold, ignore_index=ignore_index))
    ...
    def forward(self, preds: torch.Tensor, gts: torch.Tensor) -> torch.Tensor:
        ...
        for criterion, weight in zip(self.criterions, self.weights):
            if isinstance(criterion, GaussianSimilarityTMSE):
                loss += weight * criterion(preds, gts, preds)
            else:
...
class BoundaryRegressionLoss(nn.Module):
    ...
    def __init__(
        ...
        if bce:
            self.criterions.append(nn.BCEWithLogitsLoss(weight=weight, pos_weight=pos_weight))
