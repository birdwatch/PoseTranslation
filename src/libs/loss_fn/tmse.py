...
class GaussianSimilarityTMSE(nn.Module):
    ...
    def __init__(self, threshold: float = 4, sigma: float = 1.0, ignore_index: int = 255) -> None:
        super().__init__()
        self.threshold = threshold
        self.ignore_index = ignore_index
        self.mse = nn.MSELoss(reduction="none")
    ...
    def forward(self, preds: torch.Tensor, gts: torch.Tensor, sim_index: torch.Tensor) -> torch.Tensor:
        """
        Args:
